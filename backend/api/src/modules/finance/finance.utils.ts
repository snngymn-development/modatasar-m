...
export const FinanceUtils = {
   ...
   */
  calculateAccountBalance: (postings: any[]): number => {
   ...
   */
  toTRY: (amount: number, rateToTRY: number): number => {
   ...
   */
  validateDoubleEntry: (postings: any[]): boolean => {
   ...
   */
  formatTransactionKind: (kind: string): string => {
   ...
   * @returns Human-readable string
   */
  formatPostingDC: (dc: string): string => {
